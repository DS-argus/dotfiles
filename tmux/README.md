...
- `tpack`: tmux plugin 설치, 업데이트, 정리, TUI 관리
  - `Prefix I`: 설정에 선언된 플러그인 설치
  - `Prefix U`: 플러그인 업데이트
  - `Prefix M-u`: 설정에 없는 플러그인 정리
  - `Prefix T`: tpack TUI 열기
...
- `tmux source-file ~/.config/tmux/tmux.conf`: 설정 다시 로드
